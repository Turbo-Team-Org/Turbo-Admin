...
class _UserManagementPageState extends State<UserManagementPage> {
  ...
  String? _selectedRole; // Assuming roles are strings for now. Could be Role objects.
  bool _isActive = true;
  ...
  void _initializeFields(User user) {
    ...
    _emailController.text = user.email; // Email usually not editable from admin panel directly
        ...
        body: BlocConsumer<UserManagementCubit, UserManagementState>(
          ...
          builder: (context, state) {
            ...
            if (state is UserManagementLoaded) {
             ...
             if (state is UserManagementSuccess) { // Show success message and stay or allow nav
            ...
            return Center(child: Text('Error al cargar datos del usuario: ${widget.userId}'));
          },
        ),
      ),
    );
          ...
          children: <Widget>[
            Row(
                children: [
                    ...
                    SizedBox(width: 16),
                    Expanded(child: Text(user.displayName ?? 'Usuario Sin Nombre', style: theme.headlineSmall)),
                ],
            ),
            const Divider(height: 20),
            _buildDetailRow('ID Usuario:', user.id, theme),
            ...
            _buildDetailRow('Registrado:', user.createdAt != null ? DateFormat('dd MMM yyyy, hh:mm a').format(user.createdAt!) : 'N/A', theme),
  ...
  Widget _buildDetailRow(String label, dynamic value, TextTheme theme, {Color? chipColor}) {
    return Padding(
        ...
        children: <Widget>[
          ...
          Expanded(
            child: chipColor != null 
                ? Chip(label: Text(value.toString(), style: TextStyle(color: Colors.white)), backgroundColor: chipColor, padding: EdgeInsets.symmetric(horizontal: 6, vertical: 2))
  ...
  Widget _buildUserManagementForm(BuildContext context, User currentUser) {
    return Card(
        elevation: 2,
        child: Padding(
            padding: const EdgeInsets.all(16.0),
            child: Form(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  Text('Modificar Usuario', style: Theme.of(context).textTheme.titleLarge),
                  const SizedBox(height: 20),
                  TextFormField(
                    controller: _displayNameController,
                    decoration: const InputDecoration(labelText: 'Nombre Completo/Display Name', border: OutlineInputBorder()),
                    validator: (value) => (value == null || value.isEmpty) ? 'Ingresa un nombre' : null,
                  ),
                  const SizedBox(height: 16),
                  TextFormField(
                    controller: _emailController,
                    decoration: const InputDecoration(labelText: 'Email (generalmente no editable)', border: OutlineInputBorder()),
                    readOnly: true, // Email typically not directly editable by admin this way
                  ),
                  const SizedBox(height: 16),
                      ...
                      ElevatedButton.icon(
                        ...
                        onPressed: () {
                          if (_formKey.currentState!.validate()) {
                            ...
                            context.read<UserManagementCubit>().updateUserProfile(currentUser.id, profileUpdate);
