...
class UsersDataTable extends StatelessWidget {
  ...
  final Function(String userId, bool newStatus)? onUpdateStatus; // For quick activate/deactivate
  final Function(String userId, String newRole)? onUpdateRole; // For quick role change if applicable
  ...
  Widget build(BuildContext context) {
    ...
    return DataTable2(
      ...
      columns: const [
        ...
        DataColumn2(label: Text('Estado'), size: ColumnSize.S), // e.g., Active, Inactive, Banned
      ...
      rows: users.map((user) {
        ...
        return DataRow2(
          cells: [
            DataCell(
              Row(
                children: [
                  ...
                  Expanded(
                    child: Text(user.displayName ?? user.email, // Show displayName or fallback to email
            ...
            DataCell(Chip(
                label: Text(user.role?.toUpperCase() ?? 'N/A', style: TextStyle(fontSize: 10)),
                // backgroundColor: _getRoleColor(user.role).withOpacity(0.1), // Define _getRoleColor if needed
            )),
            DataCell(Text(registrationDate)),
            DataCell(
              Chip(
                label: Text(user.isActive ? 'Activo' : 'Inactivo', style: TextStyle(fontSize: 10)),
                backgroundColor: (user.isActive ? Colors.green : Colors.grey).withOpacity(0.15),
                labelStyle: TextStyle(color: user.isActive ? Colors.green.shade700 : Colors.grey.shade700),
              ),
            ),
            DataCell(
              Row(
                ...
                children: [
                  ...
                  if (onUpdateStatus != null)
                    IconButton(
                      icon: Icon(
                        user.isActive ? Icons.toggle_off_outlined : Icons.toggle_on_outlined,
                        color: user.isActive ? Colors.grey : Colors.green,
                        size: 22,
                      ),
                      tooltip: user.isActive ? 'Desactivar' : 'Activar',
                      onPressed: () => onUpdateStatus!(user.id, !user.isActive),
                    ),
                ],
              ),
            ),
